...
public class OpenOffice2PDF {
	 ...
	 * @return
	 */
	public void openOffice2Pdf(String inputFilePath, String outputFilePath) {
		...
		if ((null != inputFilePath) && (inputFile.exists()) && officeFormats.contains(getPostfix(inputFilePath))) {
			if (null == outputFilePath) {
				...
				converterFile(inputFilePath, defaultOutputFilePath, converter);
			} else {
	...
	public String generateDefaultOutputFilePath(String inputFilePath) {
		return inputFilePath.replaceAll("." + getPostfix(inputFilePath), "_" + getPostfix(inputFilePath) + ".pdf");
	}
